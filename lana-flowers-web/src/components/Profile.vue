...
const { pullDistance, refreshing, dragging } = usePullToRefresh(profileRef, loadAll)

async function removeBouquet(b) {
  if (!confirm(`Снять с продажи «${b.title}»?`)) return
  try {
...
/* Спиннер ABSOLUTE'ом — не в потоке, иначе верх аватарки уезжал под кромку. */
.ptr {
  position: absolute;
  top: -52px;
  left: 0;
...
/* WhatsApp — outlined, не доминирует над B&W темой,
   но всё ещё узнаваем благодаря зелёной иконке. */
.whatsapp-btn {
  display: flex;
  align-items: center;
  justify-content: center;
